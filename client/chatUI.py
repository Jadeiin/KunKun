...
class ChatUI(QWidget):
    def __init__(self, parent=None):
        ...
        self.ui.createGroupBtn.clicked.connect(self.addFriend)
        self.ui.addFriendLineBtn.clicked.connect(self.createGroup)
    ...
    def handleItemClicked(self, index):
        ...
        '''
        logging.debug(f"Item clicked. Index: {index}")
    ...
    def createGroup(self):
        ...
        # 上面那个那栏是群聊名输入栏吧 名字叫啥。。。
        # self.create_group_dict["roomname"] = groupNameLineEdit.toPlainText().encode()
        # send
        self.create_group = json.dumps(self.create_group_dict)
        logging.debug(self.create_group.encode())
    ...
    def additemInChatList(self, avatar_path, name, recent_msg):
        ...
        chat_widget = ChatListItemWidget(
            avatar_path, name, recent_msg, index)  # 一个新的聊天好友列表的框
    ...
    def connectItemClicked(self, chat_widget):
        ...
        logging.debug("monitoring mouse press...")
    ...
    def chatRecordScrolledToTop(self, value):
        ...
        if value == 0:
            ...
            logging.debug("Scrolled to the top!")
...
if __name__ == "__main__":
    ...
    # 替换掉 addFriendLineBtn 按键绑定
    avatar_path = "./graphSource/profPhoto.jpg"  # Replace with actual path
    name = "Paimon"  # Replace with actual name
    recent_msg = "你好"
    share.chat_page.ui.addFriendLineBtn.clicked.connect(lambda: share.chat_page.additemInChatList(
        avatar_path, name, recent_msg))  # 使用 lambda 表达式来正确地连接信号和槽
