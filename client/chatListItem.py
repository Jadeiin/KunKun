...
class ChatListItemWidget(QWidget):
    ...
    def __init__(self, avatar_path, name, recent_msg, index, parent=None):
        ...
        v_layout.addSpacerItem(QSpacerItem(
            10, 20, QSizePolicy.Fixed, QSizePolicy.Fixed))  # 用户名上方空隙，改第二个数字可调节
        v_layout.addWidget(
            name_label, alignment=QtCore.Qt.AlignTop)  # 将用户名对齐到顶部
        v_layout.addSpacerItem(QSpacerItem(
            10, 5, QSizePolicy.Fixed, QSizePolicy.Fixed))  # 用户名和最新消息之间的空隙
        recent_msg_label = QLabel(recent_msg)
        recent_msg_label.setStyleSheet("color: gray;")  # 设置最新消息标签的字体颜色为灰色
        v_layout.addWidget(
            recent_msg_label, alignment=QtCore.Qt.AlignTop)  # 将最新消息对齐到顶部
